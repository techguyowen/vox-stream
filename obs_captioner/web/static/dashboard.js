...
function populateFormFields(cfg) {
    ...
    // Audio tab
    if (cfg.general) {
        document.getElementById("engine_select").value = cfg.general.engine || "google_stt";
        document.getElementById("language_select").value = cfg.general.language || "en-US";
        toggleEngineFields(cfg.general.engine);
    }
    if (cfg.audio) {
...
document.getElementById("font_family").addEventListener("change", updatePreviewStyles);
document.getElementById("text_align").addEventListener("change", updatePreviewStyles);
document.getElementById("text_color").addEventListener("input", updatePreviewStyles);
document.getElementById("interim_color").addEventListener("input", updatePreviewStyles);
document.getElementById("highlight_color").addEventListener("input", updatePreviewStyles);
document.getElementById("bg_color_picker").addEventListener("input", updatePreviewStyles);
...
document.getElementById("btn-save-audio").addEventListener("click", async () => {
    const payload = {
        general: {
            engine: document.getElementById("engine_select").value,
    ...
    await saveConfigPayload(payload, "Audio & Engine settings saved!");
});
...
function renderTranscriptList(entries) {
    ...
    list.innerHTML = entries.map(e => `
        <div class="transcript-item">
            ...
            ${e.is_censored ? '<span class="transcript-tag">FILTERED</span>' : ''}
        </div>
    `).join("");
}
...
function appendTranscriptItem(item) {
    ...
    div.innerHTML = `
        ...
        <span class="transcript-text">${escapeHtml(item.text)} ${item.translated_text ? `<em>(${escapeHtml(item.translated_text)})</em>` : ''}</span>
        ${item.is_censored ? '<span class="transcript-tag">FILTERED</span>' : ''}
    `;
